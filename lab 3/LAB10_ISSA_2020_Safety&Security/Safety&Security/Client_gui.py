...
class Ui_MainWindow(object):
    ...
    def start_client(self):
        ...
        self.public_key = int(self.s.recv(128).decode())
        self.private_key = int(self.s.recv(128).decode())
        self.modul = int(self.s.recv(128).decode())
        print(self.public_key)
        print(self.private_key)
        print(self.modul)

    #### exercise 7 #####
        #encrypted_unlock_car=self.s.recv(128).decode()
        #print(encrypted_unlock_car)
        #self.s.send(encrypted_unlock_car.encode())
...
############################### EXERCISE 8 ###############################
    ...
    def recv_handler(self, stop_event):
        while True:
            response = self.s.recv(128).decode()
            print("RASPUNSUL DIN CLIENT ESTE " + response)
            decrypted_response=""
            if response!="error low" and response!="error high" and response!="no errors":
                decrypted_response = rsa_library.decrypt((self.private_key, self.modul),response)
                print("RASPUNSUL DECRIPTAT ESTE" + decrypted_response)
                if decrypted_response ==  '0xfd02':
...
############################### EXERCISE 9 ###############################
    def send_on_data(self):
        encrypted_airbag_on=rsa_library.encrypt((self.public_key,self.modul), airbag_on)
        self.s.send(encrypted_airbag_on.encode())

############################### EXERCISE 10 ###############################
    def send_corrupted_low(self):
        encrypted_corrupted_low = rsa_library.encrypt((self.public_key, self.modul), corrupted_low)
        self.s.send(encrypted_corrupted_low.encode())

############################### EXERCISE 11 ###############################
    def send_corrupted_high(self):
        encrypted_corrupted_high = rsa_library.encrypt((self.public_key, self.modul), corrupted_high)
        self.s.send(encrypted_corrupted_high.encode())
