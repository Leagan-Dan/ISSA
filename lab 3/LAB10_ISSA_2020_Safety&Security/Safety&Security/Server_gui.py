...
############################### EXERCISE 6 ###############################   
    def send_key_data(self):
        global conexiune
        encrypted_unlock_car = rsa_library.encrypt((public_key,modulus), unlockCar)
        print("------exercise 6:")
        print(encrypted_unlock_car)
        conexiune.send(str(encrypted_unlock_car).encode())
    ...
    def recv_messages_handler(self,stop_event):
        ...
        global errors_flag
        while True:
            message = conexiune.recv(128).decode()

            decrypted_message = rsa_library.decrypt((private_key, modulus), message)
            response = "no errors"
            if not rsa_library.low_check(decrypted_message):
                response = "error low"
            elif not rsa_library.number_check(decrypted_message):
                response = "error high"
            print(response)
            errors_flag=True
            conexiune.send(response.encode())
