...
class Server:
    ...
    def handle_message(self, command):
        ...
        if message_id== "0 2":
            ...
            print_system_notification("accept rent request")
        elif message_id == "0 5":
            print_system_notification("decline rent request")
        elif message_id == "1 6":
        ...
        elif message_id == "1 10":
            print_system_notification("request car info")
            self.request_car_information()
        elif message_id.split(' ')[0] == "11":
            print_system_notification("received car info: " + message_id.split(' ',1)[1])
        else:
            print_system_notification("not understood")

    def request_car_information(self):
        if self.car_id == -1:
            print_system_notification("no car found")
        else:
            self.car_locked = False
            self.send_bytes_to_client(self.car_id, "10")

    def unlock_car(self):
        if self.car_id == -1:
            print_system_notification("no car found car")
        else:
            if not self.car_locked:
                print_system_notification("car is already unlocked")
            else:
                self.car_locked = False
                self.send_bytes_to_client(self.car_id, "9")
    ...
    def lock_car(self):
        if self.car_id == -1:
            ...
            if self.car_locked:
                print_system_notification("car is already locked")
            else:
